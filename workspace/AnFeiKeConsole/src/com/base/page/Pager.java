...
public class Pager {
	...
	public String getOrderCondition() {
		...
		if (StringUtils.isNotBlank(this.orderField)) {
			condition = " order by " + parseOrderFields()
					+ (orderDirection ? " " : " desc ");
		}
		return condition;
	}
	
	public String parseOrderFields(){
		//多个field时
		String returnVal = "";
		if (StringUtils.isNotBlank(this.orderField)) {
			String[] orderFlds = this.orderField.split(",");
			for (String s : orderFlds){
				returnVal += ClassReflectUtil.convertToColumnName(s) + ",";
			}
