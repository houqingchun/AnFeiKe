package com.base.interceptor;

import java.io.IOException;
import java.util.Enumeration;
import java.util.HashMap;
import java.util.Map;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.apache.log4j.Logger;
import org.springframework.web.servlet.handler.HandlerInterceptorAdapter;

import com.base.util.HtmlUtil;
import com.base.util.SessionUtils;
import com.jeecg.exception.ServiceException;

/**
 * 异常拦截器
 * @author  www.jeecg.org
 *
 */
public class ExceptionInterceptor extends HandlerInterceptorAdapter {
	private final static Logger log= Logger.getLogger(ExceptionInterceptor.class);
	
	@Override
	public void afterCompletion(HttpServletRequest request,
			HttpServletResponse response, Object handler, Exception ex)
			throws Exception {
		if(ex != null){
			String msg = "";
			if (ex instanceof ServiceException) {
				msg = ex.getMessage();
			}else if (ex instanceof NullPointerException) {
				msg = "很抱歉，发生空指针异常，请联系管理员！";
			}else if (ex instanceof IOException) {
				msg = "很抱歉，文件读写发生异常，请联系管理员！";
			}else  if (ex instanceof PermissionException) {
				msg = "很抱歉，您没有权限进行此操作！";
			}else {
				msg = ex.getMessage();
			}
			log.debug("Exception msg: " + msg);
			logger(request, handler, ex);
			
			if ("AJAX".equals(request.getParameter("postType"))){
				log.debug("AJAX response[Exception Handler]");
				//以AJAX方式提交
				response.setStatus(response.SC_SERVICE_UNAVAILABLE);
				Map<String, Object> result = new HashMap<String, Object>();
				result.put("success", false);
				result.put("msg", msg);
				HtmlUtil.writerJson(response, result);
			}else{
				log.debug("Redirect response[Exception Handler]");
				//记录错误信息，以便在错误界面显示
				request.setAttribute("msg", msg);
//				response.getWriter().write(msg);
				//直接设置至提醒界面，提示需要登录
	            //response.sendRedirect(request.getContextPath() + "/error.shtml?msgCode=EXCEPTION&nextStep=&toUrl=" + request.getRequestURL() + SessionUtils.getEncodeParameters(request, true));
			}
			
		}else{
			super.afterCompletion(request, response, handler, ex);
		}
	}
	
	/**
	 * 记录日志
	 * @param request
	 * @param handler
	 * @param ex
	 */
	public void logger(HttpServletRequest request,Object handler, Exception ex){
		StringBuffer msg = new StringBuffer();
		msg.append("异常拦截日志");
		msg.append("[uri＝").append(request.getRequestURI()).append("]");
		Enumeration<String>  enumer= request.getParameterNames();
		while (enumer.hasMoreElements()) {
			String name = (String) enumer.nextElement();
			String[] values = request.getParameterValues(name);
			msg.append("[").append(name).append("=");
			if(values != null){
				int i=0;
				for(String value: values){
					i++;
					msg.append(value);
					if(i < values.length){
						msg.append("｜");
					}
					
				}
			}
			msg.append("]");
		}
		log.error(msg,ex);
	}

}
